    ...
    >
      ...
      <div class="space-y-4">
        ...
        <p class="text-stone-300 text-sm leading-relaxed">
          These are real stories from Iran's fight for freedom. To help you understand and
          empathize, we translate names, places, and numbers into your local context.
        </p>

        <!-- Examples of Context Translations -->
        <div
          class="bg-gradient-to-br from-stone-800/80 to-stone-800/40 border-2 border-stone-700/80 rounded-lg overflow-hidden"
        >
          <div class="px-5 pt-4 pb-3">
            <p class="text-stone-300 text-xs font-medium uppercase tracking-wide">
              Example of context translations
            </p>
          </div>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-t border-stone-700/50">
                  <th
                    class="px-5 py-2 text-left text-xs font-medium text-stone-400 uppercase tracking-wider"
                    >Original</th
                  >
              ...
              <tbody class="divide-y divide-stone-700/30">
                {#each modalExamples.examples as example}
                  <tr>
                    ...
                    <td class="px-5 py-3 text-primary-400 font-semibold"
                      >{getTranslation(example.id, selectedOption)}</td
                    >
                  </tr>
                {/each}
              </tbody>
      ...
      {:else}
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            >
              ...
              <svg
                class="w-5 h-5 text-stone-400 transition-transform {showDropdown
                  ? 'rotate-180'
                  : ''}"
                fill="none"
                stroke="currentColor"
              ...
              >
                {#each sortedCountries as country}
                  <button
                    ...
                    class="w-full px-4 py-3 text-left transition-colors border-b last:border-0 modal-dropdown-option {country.code ===
                    selectedOption
                      ? 'modal-dropdown-option-selected font-medium'
                      : ''}"
                  >
                    {country.name}
                  </button>
        ...
        <!-- Privacy Note -->
        <p class="text-xs text-stone-500 text-center">No tracking • Settings saved in URL</p>
      {/if}
    </div>
  </div>
</div>
